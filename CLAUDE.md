...
Test coverage is currently focused on the scoring engine (`calculatePlayerScores`, `calculateTeamScores`, `updateStandings`, `gameStateGoalBonus`), the auth layer, the read handlers that feed the season-awareness UI (`getCurrentGameweek`, `getLeagueStandings`, `getAvailableTransfers`), and the hydration guards (`hydrationCostModel`, `resolveRosterImportSeason`, `importPlayerRoster`'s sweep) — the highest-risk areas. 267 tests across 32 files as of 2026-08-20. There is still no lint tooling.
