...
Test coverage is currently focused on the scoring engine (`calculatePlayerScores`, `calculateTeamScores`, `updateStandings`, `gameStateGoalBonus`), the auth layer, the read handlers that feed the season-awareness UI (`getCurrentGameweek`, `getLeagueStandings`, `getAvailableTransfers`), the hydration guards (`hydrationCostModel`, `resolveRosterImportSeason`, `importPlayerRoster`'s sweep), and the live-poll reconciliation added 2026-08-22 (`liveMatchPolling`, `processMatchDataChanges`) — the highest-risk areas. 328 tests across 39 files as of 2026-08-22. There is still no lint tooling.

Player data hydration is two manual commands, not part of the worker cycle: `npm run hydrate:roster` (squads for the provider's *current* season — it refuses any other season without `--season-override`) and `npm run hydrate:pricing` (previews its projected API-call count and stops until `--yes`). See `docs/new-player-pricing.md`.
