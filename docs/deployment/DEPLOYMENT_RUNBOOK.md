...
| 17 | `deploy.yml`'s "Apply database migrations" step fails on its very first command: `AccessDenied … not authorized to perform: rds:AddTagsToResource on resource: arn:aws:rds:…:snapshot:fantasy-league-prod-pre-migrate-…` | `run-database-migrations.sh` passes no `--tags` of its own, but the DB instance is created with CDK's default `copyTagsToSnapshot=true`, so `CreateDBSnapshot` copies the instance's tags (`Project`/`Environment`/`ManagedBy`/`Component`) onto the new snapshot — and RDS authorizes that copy as a *separate* `rds:AddTagsToResource` call against the snapshot ARN. The `PreMigrationSnapshot` statement granted only `CreateDBSnapshot`/`DescribeDBSnapshots`. Manual local runs never hit it because they use an admin identity; only the least-privilege CI role does | Add `rds:AddTagsToResource` to the `PreMigrationSnapshot` statement, still scoped to the same db + `pre-migrate-*` snapshot ARNs | `infra/lib/githubDeployStack.ts` (fixed 2026-08-21); `cd infra && npx cdk deploy FantasyLeagueGitHubDeploy` to apply, then re-run the deploy |
